...
export default class EnphaseDevice extends Homey.Device {
  ...
  async onInit() {
    ...
    if (this.homey.platform === 'local' || this.homey.platform === 'cloud') {
      this.pollIntervalCloud = this.homey.setInterval(() => this.pollCloud(), this.constructor.POLL_INTERVAL_CLOUD);
      this.pollCloud();
    }

    if (this.homey.platform === 'local') {
      this.pollIntervalLocal = this.homey.setInterval(() => this.pollLocal(), this.constructor.POLL_INTERVAL_LOCAL);
      this.pollLocal();
    }
  }
